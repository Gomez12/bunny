...
export async function handleContactRoute(
  ...
  // ── Contact groups ───────────────────────────────────────────────────────
  const groupsListMatch = pathname.match(/^\/api\/projects\/([^/]+)\/contact-groups$/);
  if (groupsListMatch) {
    const project = decodeURIComponent(groupsListMatch[1]!);
    if (req.method === "GET") return handleListGroups(ctx, user, project);
    if (req.method === "POST") return handleCreateGroup(req, ctx, user, project);
  }

  const groupIdMatch = pathname.match(/^\/api\/projects\/([^/]+)\/contact-groups\/(\d+)$/);
  if (groupIdMatch) {
    const project = decodeURIComponent(groupIdMatch[1]!);
    const id = Number(groupIdMatch[2]);
    if (req.method === "PATCH") return handlePatchGroup(req, ctx, user, project, id);
    if (req.method === "DELETE") return handleDeleteGroup(ctx, user, project, id);
  }

  // ── Contacts list / create / import / export / edit / ask ────────────────
  const contactsListMatch = pathname.match(/^\/api\/projects\/([^/]+)\/contacts$/);
  ...
  const vcfMatch = pathname.match(/^\/api\/projects\/([^/]+)\/contacts\/(\d+)\/vcf$/);
  if (vcfMatch) {
  ...
  const contactIdMatch = pathname.match(/^\/api\/projects\/([^/]+)\/contacts\/(\d+)$/);
  if (contactIdMatch) {
...
function resolveProject(ctx: ContactRouteCtx, user: User, rawProject: string): ResolveResult {
  let project: string;
  try {
  ...
  if (!canSeeProject(p, user)) return { ok: false, error: json({ error: "forbidden" }, 403) };
  return { ok: true, project, p };
}

// ── Contact handlers ─────────────────────────────────────────────────────────

function handleList(ctx: ContactRouteCtx, user: User, rawProject: string, url: URL): Response {
  const r = resolveProject(ctx, user, rawProject);
  if (!r.ok) return r.error;

  const search = url.searchParams.get("q") || undefined;
  const groupId = url.searchParams.has("group") ? Number(url.searchParams.get("group")) : undefined;
  const limit = url.searchParams.has("limit") ? Number(url.searchParams.get("limit")) : undefined;
  const offset = url.searchParams.has("offset") ? Number(url.searchParams.get("offset")) : undefined;

  const result = listContacts(ctx.db, r.project, { search, groupId, limit, offset });
  return json(result);
}

async function handleCreate(req: Request, ctx: ContactRouteCtx, user: User, rawProject: string): Promise<Response> {
  const r = resolveProject(ctx, user, rawProject);
  if (!r.ok) return r.error;
  ...
  try {
    const contact = createContact(ctx.db, { ...body, project: r.project, name: body.name, createdBy: user.id });
    void ctx.queue.log({ topic: "contact", kind: "create", userId: user.id, data: { id: contact.id, project: r.project } });
...
function handleGet(ctx: ContactRouteCtx, user: User, rawProject: string, id: number): Response {
  const r = resolveProject(ctx, user, rawProject);
  if (!r.ok) return r.error;

  const contact = getContact(ctx.db, id);
  if (!contact || contact.project !== r.project) return json({ error: "not found" }, 404);
  return json({ contact });
}

async function handlePatch(req: Request, ctx: ContactRouteCtx, user: User, rawProject: string, id: number): Promise<Response> {
  const r = resolveProject(ctx, user, rawProject);
  if (!r.ok) return r.error;

  const contact = getContact(ctx.db, id);
  if (!contact || contact.project !== r.project) return json({ error: "not found" }, 404);
  if (!canEditContact(user, contact, r.p)) return json({ error: "forbidden" }, 403);
  ...
  try {
    ...
    void ctx.queue.log({ topic: "contact", kind: "update", userId: user.id, data: { id, project: r.project } });
...
function handleDelete(ctx: ContactRouteCtx, user: User, rawProject: string, id: number): Response {
  const r = resolveProject(ctx, user, rawProject);
  if (!r.ok) return r.error;
  ...
  void ctx.queue.log({ topic: "contact", kind: "delete", userId: user.id, data: { id, project: r.project } });
  return json({ ok: true });
}

async function handleImport(req: Request, ctx: ContactRouteCtx, user: User, rawProject: string): Promise<Response> {
  const r = resolveProject(ctx, user, rawProject);
  if (!r.ok) return r.error;

  const body = await readJson<{ contacts?: Omit<CreateContactOpts, "project" | "createdBy">[] }>(req);
  if (!body?.contacts?.length) return json({ error: "missing contacts array" }, 400);

  try {
    const imported = bulkCreateContacts(ctx.db, r.project, body.contacts, user.id);
    void ctx.queue.log({ topic: "contact", kind: "import", userId: user.id, data: { project: r.project, count: imported } });
...
function handleVcfExport(ctx: ContactRouteCtx, user: User, rawProject: string, id: number): Response {
  const r = resolveProject(ctx, user, rawProject);
  if (!r.ok) return r.error;
...
async function handleExport(req: Request, ctx: ContactRouteCtx, user: User, rawProject: string): Promise<Response> {
  const r = resolveProject(ctx, user, rawProject);
  if (!r.ok) return r.error;
...
function handleListGroups(ctx: ContactRouteCtx, user: User, rawProject: string): Response {
  const r = resolveProject(ctx, user, rawProject);
  if (!r.ok) return r.error;
  return json({ groups: listGroups(ctx.db, r.project) });
}

async function handleCreateGroup(req: Request, ctx: ContactRouteCtx, user: User, rawProject: string): Promise<Response> {
  const r = resolveProject(ctx, user, rawProject);
  if (!r.ok) return r.error;
  ...
  try {
    const group = createGroup(ctx.db, { project: r.project, name: body.name, color: body.color, createdBy: user.id });
    void ctx.queue.log({ topic: "contact", kind: "group.create", userId: user.id, data: { id: group.id, project: r.project, name: group.name } });
...
async function handlePatchGroup(req: Request, ctx: ContactRouteCtx, user: User, rawProject: string, id: number): Promise<Response> {
  const r = resolveProject(ctx, user, rawProject);
  if (!r.ok) return r.error;

  const group = getGroup(ctx.db, id);
  if (!group || group.project !== r.project) return json({ error: "not found" }, 404);
  ...
  try {
    ...
    void ctx.queue.log({ topic: "contact", kind: "group.update", userId: user.id, data: { id, project: r.project } });
...
function handleDeleteGroup(ctx: ContactRouteCtx, user: User, rawProject: string, id: number): Response {
  const r = resolveProject(ctx, user, rawProject);
  if (!r.ok) return r.error;
  ...
  void ctx.queue.log({ topic: "contact", kind: "group.delete", userId: user.id, data: { id, project: r.project, name: group.name } });
  return json({ ok: true });
}
...
async function handleEdit(req: Request, ctx: ContactRouteCtx, user: User, rawProject: string): Promise<Response> {
  const r = resolveProject(ctx, user, rawProject);
  if (!r.ok) return r.error;

  const body = await readJson<{ prompt?: string; contactsSummary?: string }>(req);
  if (!body) return json({ error: "invalid json" }, 400);
  ...
  void ctx.queue.log({ topic: "contact", kind: "edit", userId: user.id, data: { project: r.project, prompt } });
...
async function handleAsk(req: Request, ctx: ContactRouteCtx, user: User, rawProject: string): Promise<Response> {
  const r = resolveProject(ctx, user, rawProject);
  if (!r.ok) return r.error;
  ...
  void ctx.queue.log({ topic: "contact", kind: "ask", userId: user.id, data: { project: r.project, prompt, sessionId } });
