...
- **2026-04-19** — **Added user notifications.** Bell button with unread badge sits in a new `.nav__user-row` (sibling to `.nav__user` so the badge survives the hover-only opacity on the username label). New components: `<NotificationBell>` (bell + badge opens the panel on click, first click also requests OS-notification permission), `<NotificationPanel>` (floating popover anchored to the bell — row click marks-read and navigates to `deep_link`; "Mark all read" + "Load more" footer), `<ToastStack>` (fixed top-right, auto-dismiss 5 s, hover-pause), new hook `useNotifications` + `osToast` shim that routes to `@tauri-apps/plugin-notification` on desktop and `window.Notification` in the browser. New icons in the barrel: `Bell`, `BellRing`, `AtSign`. New SSE events surface `notification_created` / `notification_read` for realtime push (per-user fanout with 25 s keepalive). See [ADR 0027](./adr/0027-user-notifications.md).
