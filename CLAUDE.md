...
- **Every list/get query carries `AND deleted_at IS NULL`.** The grep audit `FROM (documents|whiteboards|contacts|kb_definitions)\b` should return zero hits without the predicate. A mangled `__trash:` name leaking into the UI is the canary if a query is ever added without the filter.
